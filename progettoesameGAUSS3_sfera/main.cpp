...
// Utilizzo GAUSS3    (ordine) p = 6  (numero stadi) ns = 3
...
const int ns=3;  //Numero degli stadi
Real b[ns];
Real c[ns];
...
int main ()
{
    ...
    ifstream myfile;
    myfile.open("puntirandomsfera.txt",ios::in);
    if (!myfile) //se il file non esiste
    {
        ...
        while(!myfile.eof()){
            ...
            Real X[d]= {0};
            copia(X,u,d);    //copia secondo elemento in entrata nel primo
